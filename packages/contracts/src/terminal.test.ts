...
describe("TerminalOpenInput", () => {
  ...
  it("accepts optional env overrides", () => {
    const parsed = decodeSync(TerminalOpenInput, {
      threadId: "thread-1",
      cwd: "/tmp/project",
      worktreePath: "/tmp/project/.hal/worktrees/feature-a",
      cols: 100,
      rows: 24,
      env: {
    ...
    expect(parsed.worktreePath).toBe("/tmp/project/.hal/worktrees/feature-a");
  });
    ...
    expect(
      decodes(TerminalEvent, {
        ...
        snapshot: {
          threadId: "thread-1",
          terminalId: DEFAULT_TERMINAL_ID,
          cwd: "/tmp/project/.hal/worktrees/feature-a",
          worktreePath: "/tmp/project/.hal/worktrees/feature-a",
          status: "running",
          pid: 1234,
          history: "",
