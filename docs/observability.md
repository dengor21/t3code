...
HAL has one server-side observability model:
...
Completed spans are written as NDJSON records to `serverTracePath` (by default, `~/.hal/userdata/logs/server.trace.ndjson`).
...
"/Applications/HAL.app/Contents/MacOS/HAL"
```
