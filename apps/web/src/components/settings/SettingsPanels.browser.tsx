...
function createBaseServerConfig(): ServerConfig {
  return {
    ...
    cwd: "/repo/project",
    keybindingsConfigPath: "/repo/project/.hal/userdata/keybindings.json",
    keybindings: [],
    issues: [],
    providers: [],
    availableEditors: ["cursor"],
    observability: {
      logsDirectoryPath: "/repo/project/.hal/userdata/logs",
...
describe("GeneralSettingsPanel observability", () => {
  ...
  it("shows diagnostics inside About with a single logs-folder action", async () => {
    ...
    await expect
      .element(page.getByText("/repo/project/.hal/userdata/logs", { exact: true }))
      .toBeInTheDocument();
    await expect
      .element(
        page.getByText(
  ...
  it("shows a disabled network access toggle with guidance in desktop builds", async () => {
    ...
    await expect
      .element(page.getByText("HAL will restart to expose this environment over the network."))
      .toBeInTheDocument();
  ...
  it("opens the logs folder in the preferred editor", async () => {
    ...
    expect(openInEditor).toHaveBeenCalledWith("/repo/project/.hal/userdata/logs", "cursor");
  });
